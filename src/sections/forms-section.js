...
const FormCard = ({ form, onUpdate, onDelete }) => {
  ...
  const addNewField = () => {
    setFields([
      ...fields,
      { title: "", placeholder: "", max: 255, key: "", required: false, isNew: true },
    ]);
  };
  ...
  const updateInput = () => {
    const createFields = fields
      .filter((field) => field.isNew && !field.isDeleted)  // New fields to be created
      .map((field) => ({
        title: field.title,
    ...
    const updateFields = fields
      .filter((field) => !field.isNew && !field.isDeleted)  // Existing fields to be updated
      .map((field) => ({
        where: { id: field.id },
        data: {
  ...
  return (
    <Card style={{ padding: 10, marginBottom: 30 }} key={form.id}>
